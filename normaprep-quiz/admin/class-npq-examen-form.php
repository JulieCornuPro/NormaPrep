 ...
 *
 * Première brique du chantier Examen : un modèle de type « scenarios ». On crée
 * un examen « vide » (nom, description, nombre de questions cible), puis on lui
 * rattache un ou plusieurs scénarios (table de liaison examen_scenario). Au
 * démarrage (étape ultérieure), l'examen tirera son nombre cible de questions
 * parmi celles des scénarios rattachés.
 *
...
class NPQ_Examen_Form {
    ...
    private static function enregistrer() {
        ...
        // Scénarios cochés : on ne garde que des ids valides (liste blanche).
        $scenarios_valides = self::ids_scenarios_valides();
        ...
        $donnees = [
            'nom'              => $nom,
            'description'      => $desc,
            'type'             => 'scenarios', // seul type géré pour cette brique
        ...
        // Réécrit la liaison scénarios (efface puis réinsère).
    ...
    public static function afficher_formulaire() {
        ...
        // Scénarios disponibles, avec le nombre de questions publiées de chacun.
        ...
        <div class="wrap">
            ...
            <form method="post" id="npq-examen-form" action="<?php echo esc_url( admin_url( 'admin.php?page=normaprep-examens' ) ); ?>">
                ...
                <p class="description">
                    Cochez les scénarios dont les questions alimentent cet examen. Le total
                    de questions disponibles est affiché ci-dessous ; il doit atteindre le
                    nombre visé (<span id="npq-cible"><?php echo (int) $nombre; ?></span>)
                ...
                <?php else : ?>
                    ...
                    <div style="max-height:360px;overflow:auto;border:1px solid #dcdcde;
                        ...
                        ?>
                            <label style="display:block;margin:.35em 0;line-height:1.4">
        ...
        <script>
        /* Additionne les questions des scénarios cochés et compare à la cible.
           Purement indicatif : aide l'admin à atteindre le nombre visé. */
        ( function () {
            var cases = document.querySelectorAll( '.npq-scenario-case' );
            ...
            function recalculer() {
                ...
                if ( alerteEl ) {
                    alerteEl.style.display = ( total < cible ) ? '' : 'none';
                }
            }

            cases.forEach( function ( c ) { c.addEventListener( 'change', recalculer ); } );
    ...
    private static function charger( $id ) {
        ...
        return $wpdb->get_row( $wpdb->prepare(
            "SELECT id, nom, description, type, nombre_questions, actif
    ...
    /**
     * Scénarios publiés de la certification courante, avec le nombre de
     * questions publiées de chacun.
     */
    private static function scenarios_avec_compte() {
        ...
        return (array) $wpdb->get_results( $wpdb->prepare(
            "SELECT s.id, s.nom,
             ...
             FROM {$p}scenario s
             WHERE s.certification_id = %d AND s.statut = 'publie'
             ORDER BY s.nom ASC",
            $certification_id
        ), ARRAY_A );
    }

    /** Liste blanche des ids de scénarios publiés (certification courante). */
    private static function ids_scenarios_valides() {
        return array_map(
            function ( $s ) { return (int) $s['id']; },
            self::scenarios_avec_compte()
        );
    }
